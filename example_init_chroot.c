...
void _start() {
    sys_c_mkdir("/proc", 0755);
    sys_c_mount("proc", "/proc", "proc", MS_NOSUID | MS_NODEV | MS_NOEXEC | MS_NOATIME | MS_SILENT, NULL);
    // sys_c_write(sys_c_open("/proc/self/fd/0" /*STDIN*/, O_WRONLY, NULL), "test to console\n");
    // sys_c_write(sys_c_open("/proc/self/fd/1" /*STDOUT*/, O_WRONLY, NULL), "test to console\n");
    // sys_c_write(sys_c_open("/proc/self/fd/2" /*STDERR*/, O_WRONLY, NULL), "test to console\n");
    ...
    sys_c_mount(FS, FS_ROOT, FS_TYPE, MS_RDONLY | MS_NOATIME | MS_SILENT, NULL);
    sys_c_chdir(FS_ROOT);
    sys_c_mount("/dev", "dev", NULL, MS_MOVE | MS_SILENT, NULL);
    sys_c_mount("/proc", "proc", NULL, MS_MOVE | MS_SILENT, NULL);
